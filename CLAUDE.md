...
CSS best practices documentation site, built with zudo-doc (zfb framework, MDX, Tailwind CSS v4).
...
7. Run `pnpm b4push` to verify the site builds correctly
...
`@takazudo/zudo-doc` ships **no precompiled CSS** — its header/sidebar/doclayout/admonition styles are static Tailwind class literals inside its compiled `dist/**/*.js`, so this consumer must generate them itself. The package ships a build-generated safelist at `dist/safelist.css` (an `@source inline(...)` of every class token it emits, exported as `./safelist.css`), and `src/styles/global.css` pulls it in with `@import "@takazudo/zudo-doc/safelist.css"`. Importing through the module resolver (same code path as `@import "@takazudo/zdtp/styles.css"`) is reliable and auto-syncs on package upgrade — no mirror, no hand-maintained safelist, no `@source` over `node_modules` (Tailwind v4 does not reliably traverse pnpm's symlinked `node_modules`, which silently dropped ~28% of the package's classes — the reason the old dist-mirror workaround existed).

- The safelist tells Tailwind to *generate* each token, but a `shadow-*` / `tracking-*` / `*-vsp-3xs` utility only materializes if this consumer's `@theme` defines the matching token. `--shadow-lg` and `--spacing-vsp-3xs` are defined in `global.css` (matching the canonical scaffold). `tracking-wider` and bare `rounded` are intentionally **not** defined — the canonical scaffold defines no `--tracking-*` / bare `--radius` either, so leaving them ungenerated is dogfood parity, not a gap.
- `page-loading-overlay` / `page-loading-spinner` are semantic component classes (not utilities) for which the package ships no CSS rule — Tailwind cannot generate them; that is a separate upstream concern.
...
- Base path: `/` (root — served at `https://zudo-css.takazudomodular.com/`)
