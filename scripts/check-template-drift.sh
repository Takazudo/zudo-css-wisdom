...
# Only check features that were scaffolded for THIS site (zcss).
# Features not scaffolded — designTokenPanel, docTags, tagGovernance, tauri,
# tauriDev, versioning — are intentionally absent; for those we ship the base
# no-op stubs (e.g. design-token-panel-bootstrap.tsx is the base stub, already
# covered by the base/ pass above). Listing an unscaffolded feature here would
# produce spurious [MISSING IN PROD] errors for its files.
#
# Feature name -> template directory mapping (scaffolded for zcss):
...
#                       NOTE: zcss uses the package export
