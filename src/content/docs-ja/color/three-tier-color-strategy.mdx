...
- [カスタムプロパティ上級編](../custom-properties/index.mdx) — フォールバックチェーン、スペーストグル、3層パターンを実装するコンポーネント API
- [テーマレシピ](../custom-properties/theming-recipes.mdx) — このアーキテクチャを使った本番向けテーマレシピ
- [カラートークンパターン](../design-tokens/tight-token-strategy/color-tokens.mdx) — Tailwind の `@theme` システムにおける3層トークンの適用
- [3層フォントサイズ戦略](../font-sizing/three-tier-font-size-strategy.mdx) — 同じ3層アーキテクチャをフォントサイズに適用する方法
