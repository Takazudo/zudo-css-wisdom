---
title: カスタムプロパティ
sidebar_label: カスタムプロパティ
sidebar_position: 20
description: 単純な var() の置き換えにとどまらないカスタムプロパティ：スペーストグルトリック、フォールバックチェーン、コンポーネントのスタイリング API、@property による型付きプロパティ。
---

import CssPreview from '@/components/CssPreview';

カスタムプロパティは、色に一度名前を付けて使い回すだけのものではありません。このページではスペーストグル、フォールバックチェーン、コンポーネントのスタイリング API といった応用的な仕組みを扱い、以下のページではそれぞれをパターンカタログ、完全なテーマレシピ、`@property` で登録する型付きプロパティへとさらに掘り下げます。
...
コンポーネントが段階的に広いデフォルトを確認するレイヤード設定システムを作成します。これは[3層カラー戦略](../color/three-tier-color-strategy.mdx)の背後にあるメカニズムです — コンポーネントトークンはテーマトークンにフォールバックし、テーマトークンはパレット値にフォールバックします。
...
- [パターンカタログ](./pattern-catalog.mdx) — インタラクティブなデモ付きのCSSカスタムプロパティパターンの包括的なコレクション
- [テーマレシピ](./theming-recipes.mdx) — ライト/ダークモード、ブランドテーマ、コンポーネントライブラリのための完全なテーマシステムレシピ
- [@property](./at-property.mdx) — 型付きのカスタムプロパティを登録し、アニメーションと補間を可能にしつつ、失敗時も安全に振る舞わせる
