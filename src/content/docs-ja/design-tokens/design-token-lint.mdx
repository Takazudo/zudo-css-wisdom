...
- `z-index: 99` のショートカットが [tier system](./z-index-strategy.mdx) を回避します。一度すり抜けたら、次の貢献者は `99` を見て `100` を書きます。
...
Pass 3（生 `z-index` 整数の禁止）は **計画段階** であり、まだ出荷されていません。[Z-Index Strategy](./z-index-strategy.mdx) のティアシステムと一緒に投入される予定です。アップストリームの状況は [zudo-pattern-gen#942](https://github.com/zudolab/zudo-pattern-gen/issues/942) で追跡してください。契約を明確にするためルールはいま文書化し、実装はアップストリームの準備が整ったタイミングで投入します。
...
コンポーネント CSS の生 `z-index: <integer>` は、[Z-Index Strategy](./z-index-strategy.mdx) のティアシステムを掘り崩します。Pass 3 はそれを禁止します。
...
姉妹記事 [Z-Index Strategy](./z-index-strategy.mdx) は Pass 3 が強制するティアトークンシステムを文書化しています。Pass 3 の状況: [zudo-pattern-gen#942](https://github.com/zudolab/zudo-pattern-gen/issues/942) で計画中。
...
およそ 3,000 箇所のスペーシング使用を対象としたある移行での実測: 5 つの領域ファイルにまたがる 31 件のベースラインエントリ → 恒久的な生き残りは **2** 件（どちらも `mx-[0.25em]`、原理的に例外となる em ベースのインライン微調整）、1,042 ファイルで lint はクリーン、6 つの並行 worktree にまたがって許可リストのマージコンフリクトはゼロ。周辺のアーキテクチャについては [高密度なデスクトップクラス Web アプリのスペーシングアーキテクチャ](./app-spacing-architecture.mdx) を参照してください。
...
**ズームからオプトアウトする固定 px のサイジング任意値** ―― ディスプレイスケールを持つアプリでは、`w-[28px]` は [2層サイズ戦略](./two-tier-size-strategy.mdx) の契約上許可されており、その寸法をあらゆるズームレベルで固まらせます。監査したあるプロジェクトの lint は 1,042 ファイルでグリーンでしたが、その裏でスケーリングしない閉じるボタンのリテラルが 41 件出荷されていました。
...
- [Z-Index Strategy](./z-index-strategy.mdx) ―― 姉妹メソドロジー記事（Pass 3 のティアシステム）
- [Tight Token Strategy](./tight-token-strategy/index.mdx) ―― Pass 1 が強制するトークン規約
- [Multi-Namespace Token Strategy](./multi-namespace-token-strategy.mdx) ―― 複数のデザインコンテキストを持つプロジェクトのための命名
