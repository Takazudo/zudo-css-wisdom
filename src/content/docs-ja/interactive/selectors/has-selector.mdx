...
スタッキングはすべてのページに適用されるようになりました。`:has()`のゲートが残るのは、条件が本物である場所 — コードパネルの存在によって生じる調整 — だけです。

<CssPreview client:load
  title="修正版：無条件のスタッキング、パネル固有の差分のみゲート"
  html={`
...
`:has(#feature)`のゲートが、その機能自身ではなく他の要素 — サイドバーやメインカラム — をスタイリングしている状態です。ルールをゲートする前に、「その機能が存在しないとき、このルールは正しくなくなるか」を自問してください。レスポンシブなスタッキング、最小サイズのリセット、オーバーフロー対策はすべてのページで成立しなければならないため、答えは常にノーです。
