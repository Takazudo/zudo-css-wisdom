...
- [position: fixed の包含ブロック](./containing-block-for-fixed.mdx) ―― このバグの“沈黙の双子”。ほぼ同じトリガープロパティ群（`transform`、`filter`、`backdrop-filter`、`perspective`、`will-change`、`contain`、`content-visibility`）が `position: fixed` の子孫に対して**新しい包含ブロック**も作成し、子孫がビューポートではなくその祖先のボックスに対してピン留めされるようになります。
- [Z-Index 戦略](../../methodology/design-systems/z-index-strategy.mdx) ―― スタッキングコンテキストの理解が済んだら、z-index 値そのものは単一の真実のソースから生成されるセマンティックトークン（`--z-modal`、`--z-tooltip`、`--z-local-N`）と、生整数を禁ずる lint ルールで管理します。
