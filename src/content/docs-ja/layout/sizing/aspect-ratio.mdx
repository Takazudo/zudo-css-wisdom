...
`object-fit` は置換要素（`img`、`video`、`iframe`）にしか効きません。任意の要素 — スライド、ゲームキャンバス、埋め込みアプリなど — を境界のあるボックスに正しい軸のバー付きで収めるには、`max-height: 100%` を足しても実現できません。[レターボックスステージ](./letterbox-stage.mdx)を参照してください。
