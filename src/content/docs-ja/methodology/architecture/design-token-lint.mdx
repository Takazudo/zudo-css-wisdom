...
数値サイジングスケールはポリシー上禁止されています。現在のリファレンス実装ではその一部しか強制できません ―― 詳細は後述の「リファレンス実装」セクションのツール面のギャップに関する注記を参照してください。
...
- 衝突のない数値サイジングプレフィックス（`size-{n}`、`min-w-{n}`、`max-w-{n}`、`min-h-{n}`、`max-h-{n}`）を禁止 ―― 詳細は後述のツール面のギャップに関する注記を参照してください
...
Tailwind の不透明度修飾子（`bg-black/50`）は **カラー** ユーティリティにのみ適用されます。`w`、`h`、`size`、`basis` には存在しません ―― サイジングユーティリティの末尾の `/N` は常にフラクションであり、不透明度の値になることはありません。修正の方向性としては、ベースのユーティリティがカラーユーティリティである場合にのみ、linter が末尾の `/N` を不透明度修飾子として取り除くようにすることです。サイジングユーティリティであれば `1/2` が裸の数値パターンにマッチすることはなくなり、`w-1/2` は正しく通過するようになります。

それがアップストリームに反映されるまでは、禁止できるのは衝突のないサイジングプレフィックスだけです: `size-{n}`、`min-w-{n}`、`max-w-{n}`、`min-h-{n}`、`max-h-{n}`。`w-{n}`、`h-{n}`、`basis-{n}` は禁止リストに含めないでください ―― 現行の v1.0.0 のマッチャーでこれらを禁止すると、これらのプレフィックス上に構築されたすべてのフラクション（`w-1/2`、`h-2/3`、`basis-1/3`）が誤検知されてしまいます。
