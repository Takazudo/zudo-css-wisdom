...
- [Z-Index 戦略](../design-tokens/z-index-strategy.mdx) ―― スタッキングコンテキストの理解が済んだら、z-index 値そのものは単一の真実のソースから生成されるセマンティックトークン（`--z-modal`、`--z-tooltip`、`--z-local-N`）と、生整数を禁ずる lint ルールで管理します。
