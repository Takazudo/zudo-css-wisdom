...
- `z-index: 99` shortcuts bypass the [tier system](./z-index-strategy.mdx). Once one slips in, the next contributor sees `99` and writes `100`.
...
Pass 3 (raw `z-index` integers) is **planned**, not shipped. It will land alongside the [Z-Index Strategy](./z-index-strategy.mdx) article's tier system. Track upstream status at [zudo-pattern-gen#942](https://github.com/zudolab/zudo-pattern-gen/issues/942). Document the rule now so the contract is clear; ship the implementation when the upstream is ready.
...
Raw `z-index: <integer>` in component CSS undermines the [Z-Index Strategy](./z-index-strategy.mdx) tier system. Pass 3 forbids them.
...
The companion [Z-Index Strategy](./z-index-strategy.mdx) article documents the tier-token system that Pass 3 enforces. Pass 3 status: planned at [zudo-pattern-gen#942](https://github.com/zudolab/zudo-pattern-gen/issues/942).
...
- [Z-Index Strategy](./z-index-strategy.mdx) — companion methodology article (Pass 3's tier system)
- [Tight Token Strategy](./tight-token-strategy/index.mdx) — the token convention that Pass 1 enforces
- [Multi-Namespace Token Strategy](./multi-namespace-token-strategy.mdx) — naming for projects with several design contexts
