...
Defining a tight token set is half the work. The other half is preventing raw values from sneaking back into component code as the codebase grows. See [Design Token Lint](../design-token-lint.mdx) for the multi-pass linter pattern that enforces a tight token set at pre-push and CI time.

## Deep Dive

For detailed token strategies in specific categories:

- [Color Token Patterns](./color-tokens.mdx) — Semantic color scales, brand colors, state colors, and surface layers (see also [Three-Tier Color Strategy](../../color/three-tier-color-strategy.mdx) for the underlying architecture)
