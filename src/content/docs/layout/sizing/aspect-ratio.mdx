...
`object-fit` only works on replaced elements such as `img` and `video` — it has no effect on `iframe`. To get the same `contain` behavior for an arbitrary element — a slide, a game canvas, or an embedded app that must fit a bounded box with bars on the correct axis — adding `max-height: 100%` does not work. See [Letterbox Stage](./letterbox-stage.mdx).
