...
Grid and flex tracks have two separate failure layers when content is oversized: geometry and paint. `min-height: 0` (or `min-width: 0`) fixes geometry — the track stops growing to fit its content. It does nothing about paint: the item's content still overflows its box with the default `overflow: visible`, and because inline content paints in a later phase than sibling backgrounds, the text renders on top of the next row.
