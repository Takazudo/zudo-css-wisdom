...
The fixed `width`/`height` per unit must match the `@page` size (minus any `@page` margin) so each unit fills exactly one page. When both come from the same constants table (see the next two sections), they cannot drift apart.
...
| Mid-animation frames printed | `animation-duration: 0.001s`, `animation-delay: 0s`, `transition: none` |
