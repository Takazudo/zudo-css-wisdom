...
The numeric sizing scale is banned in policy. Today's reference implementation can only enforce part of it — see the tooling-gap note in the Reference implementation section below.
...
- Forbids the conflict-free numeric sizing prefixes (`size-{n}`, `min-w-{n}`, `max-w-{n}`, `min-h-{n}`, `max-h-{n}`) — see the tooling-gap note below
...
Tailwind opacity modifiers (`bg-black/50`) apply only to **color** utilities. They do not exist for `w`, `h`, `size`, or `basis` — a trailing `/N` on a sizing utility is always a fraction, never an opacity value. The fix direction is for the linter to strip a trailing `/N` as an opacity modifier only when the base utility is a color utility; for sizing utilities, `1/2` would then never match a bare-number pattern and `w-1/2` would correctly pass.

Until that lands upstream, only the conflict-free sizing prefixes are bannable: `size-{n}`, `min-w-{n}`, `max-w-{n}`, `min-h-{n}`, `max-h-{n}`. Keep `w-{n}`, `h-{n}`, and `basis-{n}` off the ban list — banning them with the current v1.0.0 matcher would false-positive every fraction built on those prefixes (`w-1/2`, `h-2/3`, `basis-1/3`).
