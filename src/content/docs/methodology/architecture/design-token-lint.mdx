...
| --- | --- | --- |
| Numeric spacing scale | Ban | `p-4`, `m-8`, `gap-6`, `inset-2`, ... |
| Numeric sizing scale | Ban | `w-24`, `h-3`, `size-2`, `min-w-4`, `max-w-8`, ... |
| Default color palette | Ban | `bg-gray-500`, `text-blue-600`, `border-red-400`, ... |
| Semantic tokens | Allow | `px-hsp-sm`, `py-vsp-md`, `bg-surface`, `text-ink` |
| Arbitrary values (escape hatch for one-off, component-specific values) | Allow | `w-[30px]`, `p-[6px]`, `grid-cols-[120px_1fr]`, `bg-[hsl(...)]` |
| Fractions (proportional, NOT the freedom scale) | Allow | `w-1/2`, `w-1/3`, `basis-2/3` |
| Keywords | Allow | `w-full`, `h-screen`, `max-w-prose`, `min-w-0` |
| Zero | Allow | `p-0`, `w-0` |
...
1. **Land the rule plus a full current-violation baseline, green on day one.** Every existing violation goes into an allowlist keyed by file → exact class string. CI enforces "no *new* violations" from the first commit, while the backlog is paid down incrementally.
...
3. **Auto-accept the golden path by pattern, not by entry.** When an escape form is *always* sanctioned, encode it in the rule itself. Any bracket value containing `var(--display-scale)` or `var(--spacing-*)` — plus `env()`/vh/vw/% viewport math and 1px structural hairlines, including `min()`/`max()` wrappers of those — passes without an allowlist line. If every correct escape needed an entry, the allowlist would grow at exactly the rate the codebase improves.
...
<Warning title="Fraction vs. opacity-modifier conflict (v1.0.0)">
...
<Warning title="Digit-prefixed token names break naive numeric bans">
...
A green design-token lint proves values, not layouts. Two defect classes ship straight through it.

**Proximity and ratio failures** — every value legal, the relationship illegible. A linter checks values in isolation, so a card list whose between-card gap equals its within-card rhythm passes every rule while reading as one fused blob (see [Spacing Philosophy](../design-principles/spacing-philosophy.mdx)).

**Fixed-px sizing arbitraries that opt out of zoom** — in a display-scaled app, `w-[28px]` is allowed by the [Two-Tier](../design-systems/two-tier-size-strategy.mdx) contract and freezes that dimension at every zoom level. One audited project's lint was green across 1,042 files while 41 non-scaling close-button literals shipped.

Pair the lint gate with computed-style verification predicates and a squint pass at multiple display scales. The lint is a floor, not a ceiling — it proves nobody wrote an illegal value, which is a much weaker claim than "the UI is correct."
