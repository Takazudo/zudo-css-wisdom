...
Until that lands upstream, the numeric sizing scale cannot be safely banned by a bare `{prefix}-{n}` rule for any of these prefixes — banning any of `w-{n}`, `h-{n}`, `size-{n}`, `min-w-{n}`, `max-w-{n}`, `min-h-{n}`, `max-h-{n}`, or `basis-{n}` with the current v1.0.0 matcher would false-positive a real fraction built on that prefix. The numeric spacing scale (`p-{n}`, `m-{n}`, `gap-{n}`, `space-x-{n}`, …) has no fraction syntax in Tailwind and remains safely bannable today.
