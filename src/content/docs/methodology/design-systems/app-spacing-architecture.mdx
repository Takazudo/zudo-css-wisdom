...
| Common region | how to group when the within-group rhythm is not yours to set |
...
<CssPreview client:load
  title="Intrinsic Rhythm: Gap Ratio Fails, Common Region Wins"
  html={`
<div class="fg-demo">
...
| `card-gap` | gap between bounded cards on board-like surfaces | pairs with the common-region recipe |
...
| Grouping fails but within-rhythm is line-height | Common region: a bounded box (border + radius + fill + gap), not more margin |
