...
In a Tailwind v4 project with the [tight token strategy](./tight-token-strategy/index.mdx), declare base values in `@theme` and override them with scaled values in `:root`:
