...
For a detailed look at how this applies specifically to width/height sizing — where the abstract scale layer is intentionally skipped — see [Two-Tier Size Strategy](../../two-tier-size-strategy/). For how the same token-vs-arbitrary judgment plays out at the scale of a whole app's chrome — where one arbitrary value copy-pasted dozens of times is the signal an unnamed role token is missing — see [Spacing Architecture for Dense Desktop-Class Web Apps](../../app-spacing-architecture.mdx).
