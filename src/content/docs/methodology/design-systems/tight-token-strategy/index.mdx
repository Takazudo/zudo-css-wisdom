...
Tailwind CSS ships with an enormous default token set, and since v4 the spacing scale is not even a fixed list. Spacing utilities compute from a single `--spacing` variable — `padding: calc(var(--spacing) * <number>)`, with `--spacing` defaulting to `0.25rem`. Any numeric multiple works: `p-4`, `p-13`, `p-17`, and `gap-21` are all valid without touching a config file. (v3 shipped a fixed scale of roughly 33 steps; v4 removed that ceiling.) Multiply this open-ended spacing axis by colors, font sizes, border radii, and other categories, and the available utility space is effectively unlimited.
...
Defining a tight token set is half the work. The other half is preventing raw values from sneaking back into component code as the codebase grows. See [Design Token Lint](../../architecture/design-token-lint.mdx) for the multi-pass linter pattern that enforces a tight token set at pre-push and CI time.

## Deep Dive

For detailed token strategies in specific categories:

- [Color Token Patterns](./color-tokens.mdx) — Semantic color scales, brand colors, state colors, and surface layers (see also [Three-Tier Color Strategy](../../../styling/color/three-tier-color-strategy.mdx) for the underlying architecture)
- [Typography Token Patterns](./typography-tokens.mdx) — Font size, line-height, font-weight, and letter-spacing token strategies
- [Token Preview](./token-preview.mdx) — Visual reference of all available tokens
- [Component Tokens & Arbitrary Values](./component-tokens.mdx) — When to use system tokens vs arbitrary values
- [Two-Tier Size Strategy](../two-tier-size-strategy.mdx) — Why width/height sizing skips the abstract layer and uses semantic theme tokens directly
