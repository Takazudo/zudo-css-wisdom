...
- [Z-Index Strategy](../design-tokens/z-index-strategy.mdx) — once the stacking-context rules are clear, manage z-index values themselves with semantic tokens (`--z-modal`, `--z-tooltip`, `--z-local-N`) generated from a single source of truth, plus a lint rule that forbids raw integers.
