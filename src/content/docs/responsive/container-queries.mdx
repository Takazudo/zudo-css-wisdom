...
For the `clamp()` patterns that combine these units into fluid font sizing — additive (`rem` + `cqi`) vs. pure (`cqi` only) — see [Fluid Font Sizing with clamp()](../font-sizing/fluid-font-sizing.mdx).
...
A fixed-aspect stage is the typical host for this pattern — see [Letterbox Stage](../media/letterbox-stage.mdx) for the container setup itself. Derive each `clamp()` slope with the same math as viewport-based fluid type — see [Fluid Font Sizing with clamp()](../font-sizing/fluid-font-sizing.mdx). Keep the scale to a handful of steps: the discipline arguments in [Tight Token Strategy](../design-tokens/tight-token-strategy/index.mdx) apply unchanged to component-scoped tokens.
...
Rule of thumb: at the width the component is designed for, the `rem` term contributes at least ~60% of the resolved preferred value. One deliberate exception: fixed-composition stages (slides, deck viewers) want pure-cqi proportional type instead — see [Stage-Proportional Typography](../font-sizing/stage-proportional-typography.mdx). Verify by resolving the preferred expression at the design width:
