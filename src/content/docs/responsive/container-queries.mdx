...
For the `clamp()` patterns that combine these units into fluid font sizing — additive (`rem` + `cqi`) vs. pure (`cqi` only) — see [Fluid Font Sizing with clamp()](../typography/font-sizing/fluid-font-sizing.mdx).
...
A fixed-aspect stage is the typical host for this pattern — see [Letterbox Stage](../layout/sizing/letterbox-stage.mdx) for the container setup itself. Derive each `clamp()` slope with the same math as viewport-based fluid type — see [Fluid Font Sizing with clamp()](../typography/font-sizing/fluid-font-sizing.mdx). Keep the scale to a handful of steps: the discipline arguments in [Tight Token Strategy](../methodology/design-systems/tight-token-strategy/index.mdx) apply unchanged to component-scoped tokens.

Three rules keep such a scale from failing.

### Keep the rem Term Dominant

Browser zoom scales `rem` values; it does not scale `cqi` values — zooming does not make the container wider, so the `cqi` term resolves to the same length while the `rem` term grows. In a token where the `cqi` term dominates, user zoom barely changes the text size. That fails WCAG 1.4.4 (Resize Text), which requires text to stay functional scaled up to 200%.

Rule of thumb: at the width the component is designed for, the `rem` term contributes at least ~60% of the resolved preferred value. Verify by resolving the preferred expression at the design width:
...
<CssPreview client:load
  title="rem-dominant vs cqi-dominant tokens under simulated 200% text zoom"
  html={`
    <div class="zoom-sim">
...
<CssPreview client:load
  title="One cqi token, consumed inside vs outside its container — switch viewports"
  html={`
    <div class="ctx-demo">
...
<CssPreview client:load
  title="Fluid cqi hairline vs fixed 1px hairline — drag the stage narrower"
  html={`
...
| --- | --- |
| `rem` term at ~60% or more of the resolved value at the design width | Text that ignores browser zoom (WCAG 1.4.4) |
| cq tokens consumed only inside their container's subtree | Absurd spacing in chrome at extreme widths |
| Hairlines stay literal `px` | Borders and rules fading out in narrow containers |
...
- **Letting the `cqi` term dominate a fluid token**: Browser zoom scales `rem`, not `cqi`, so a cqi-dominant `clamp()` barely responds to user zoom. Keep the `rem` term at ~60% or more of the resolved value at the design width.
