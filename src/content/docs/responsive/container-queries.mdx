...
Rule of thumb: at the width the component is designed for, the `rem` term contributes at least ~60% of the resolved preferred value. One deliberate exception: fixed-composition stages (slides, deck viewers) want pure-cqi proportional type instead — see [Stage-Proportional Typography](../typography/font-sizing/stage-proportional-typography.mdx). Verify by resolving the preferred expression at the design width:
...
| --- | --- |
| `rem` term at ~60% or more of the resolved value at the design width | Text that ignores browser text scaling (WCAG 1.4.4) |
