...
:::warning[Define leads in `rem`, not `em`]
...
- **Writing the shared lead in `em`.** The float and the heading have different font sizes, so `2em` is a different number of pixels on each. Use `rem` or fixed tokens for `--flow-space` values.
