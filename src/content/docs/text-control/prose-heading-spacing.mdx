...
:::warning[Floats break this strategy]
A floated child is positioned by its **own** `margin-block-start`, so it top-aligns with the block beside it only when both leads are equal — and a heading's lead is deliberately larger. See [Float and Flow Spacing](./float-and-flow-spacing.mdx) for the mechanism, the backwards "zero the follower" anti-pattern, and the fix.
:::
...
| Floated figure beside a heading | Float adopts the heading's lead | `.figure:has(+ h3) { --flow-space: 2em }` — see [Float and Flow Spacing](./float-and-flow-spacing.mdx) |
...
- **Zeroing the lead of the block after a float.** The float already carries its own lead, so zeroing the neighbour moves the text *above* the image. Equalise the two leads instead — see [Float and Flow Spacing](./float-and-flow-spacing.mdx).
