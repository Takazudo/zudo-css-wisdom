...
This article builds on [Fluid Font Sizing with clamp()](./fluid-font-sizing). Make sure you understand basic `clamp()` usage before reading further.
...
Prefer a single `clamp()` (covered in [Fluid Font Sizing](./fluid-font-sizing)) when:
